...
import 'package:gallery_picker_plus/models/gallery_album.dart';
...
class AlbumAppBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  const AlbumAppBar({super.key, required this.album, required this.controller, required this.isBottomSheet});
      ...
      actions: [
        !controller.pickerMode
            ? TextButton(
                ...
                child: Icon(Icons.check_box_outlined, color: controller.config.appbarIconColor),
              )
            : const SizedBox(),
      ],
    );
  ...
  Widget getTitle() {
    if (!controller.pickerMode && controller.selectedFiles.isEmpty) {
      return Text(album.name ?? "Unnamed Album", style: controller.config.appbarTextStyle);
    } else if (controller.pickerMode && controller.selectedFiles.isEmpty) {
      return Text(controller.config.tapPhotoSelect, style: controller.config.appbarTextStyle);
