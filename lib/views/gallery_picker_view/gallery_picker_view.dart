...
class GalleryPickerView extends StatefulWidget {
  ...
  final Widget Function(String tag, MediaFile media, BuildContext context)? heroBuilder;
  final Widget Function(List<MediaFile> media, BuildContext context)? multipleMediaBuilder;
...
class _GalleryPickerState extends State<GalleryPickerView> {
                            ...
                            child: Row(
                              ...
                              children: [
                                _buildMenuButton(
                                  ...
                                  onPressed: () {
                                    galleryController.pickerPageController.animateToPage(
                                      0,
  ...
  Widget _buildMenuButton({required String title, required bool isSelected, required VoidCallback onPressed}) {
    return Expanded(
      child: Container(
        height: 48,
        decoration: isSelected
            ? BoxDecoration(
                border: Border(bottom: BorderSide(color: config.underlineColor, width: 3.0)),
              )
            : null,
        child: TextButton(
          onPressed: onPressed,
          child: Text(title, style: isSelected ? config.selectedMenuStyle : config.unselectedMenuStyle),
        ),
      ),
    );
  }
