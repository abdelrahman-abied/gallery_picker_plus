...
import 'package:gallery_picker_plus_example/examples/multiple_medias.dart';
      ...
      body: Stack(
        children: [
          if (cameraController != null && cameraController!.value.isInitialized)
            SizedBox(height: MediaQuery.of(context).size.height, child: CameraPreview(cameraController!)),
                        ...
                        child: GestureDetector(
                          onTap: () {
                            if (selectedMedias.isEmpty) {
                              Navigator.push(
                                context,
                                MaterialPageRoute(builder: (context) => MultipleMediasView([media])),
                              );
                            } else {
                              selectedMedias.any((element) => element.id == media.id)
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              border: Border.all(
                                width: 2,
                                color: selectedMedias.any((element) => element.id == media.id)
                                    ? Colors.red
                                    : Colors.black,
                              ),
                            ),
                            child: Stack(
                              children: [
                                SizedBox(width: 65, height: 65, child: ThumbnailMedia(media: media)),
                                if (selectedMedias.any((element) => element.id == media.id))
                                  Container(
          ...
          if (selectedMedias.isNotEmpty)
            Positioned(
              ...
              child: FloatingActionButton(
                onPressed: () {
                  Navigator.push(context, MaterialPageRoute(builder: (context) => MultipleMediasView(selectedMedias)));
                },
                mini: true,
                ...
                children: [
                  ...
                  TextButton(
                    ...
                    child: Container(
                      decoration: BoxDecoration(color: Colors.black.withOpacity(0.5), shape: BoxShape.circle),
                      padding: const EdgeInsets.all(8),
                      child: const Icon(Icons.cameraswitch, size: 20, color: Colors.white),
                    ),
                  ),
                ],
              ),
